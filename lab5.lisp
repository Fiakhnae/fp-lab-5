...
(defun pretty-print-alist (alist)
  (if (null alist)
      ...
      (let ((key-width (reduce #'max (mapcar (lambda (pair) 
                                               (length (prin1-to-string (car pair))))
                                             alist))))
        (dolist (pair alist)
          (let ((key (car pair))
                (value (cdr pair)))
            (if (and (listp value) (listp (car value)))
                (progn
                  ...
                  (pretty-print-nested-alist value key-width))
...
(defun test-read-display-data ()
  (let ((manufacturers (select "Manufacturers.csv"))
    ...
    (pretty-print-alist (funcall manufacturers))
    (format t "~%Drones data:~%")
    (pretty-print-alist (funcall drones))))
...
(defun test-filter-data ()
  (let ((usa-manufacturers (select "Manufacturers.csv" :Country "USA"))
    ...
    (pretty-print-alist (funcall usa-manufacturers))
    (format t "~%Drones for photography:~%")
    (pretty-print-alist (funcall photography-drones))))

(defun test-data-transformation ()
  (let* ((drones (select "Drones.csv"))
         (drone (first (funcall drones))) 
